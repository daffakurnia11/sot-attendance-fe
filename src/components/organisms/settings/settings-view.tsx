...
const fields = [
  ...
  { key: "start_date_contract", label: "Contract start date", help: "Day of month when each attendance and payslip period starts.", placeholder: "28" },
...
export function SettingsView({ initialCharacterName, initialData }: Props) {
  ...
  async function saveProfile() {
    ...
    if (!parsed.success) { setProfileFeedback({ type: "error", message: t("Character name must contain 1 to 80 characters.") }); return; }
    ...
    try {
      ...
      if (!response.ok) throw new Error(typeof payload === "object" && payload && "error" in payload && typeof payload.error === "string" ? payload.error : t("Profile could not be saved."));
      const updated = memberProfileSchema.parse(payload); setCharacterName(updated.character_name); setProfileFeedback({ type: "success", message: t("Character name saved.") });
    } catch (error) { setProfileFeedback({ type: "error", message: translate(error instanceof Error ? error.message : t("Profile could not be saved.")) }); }
  ...
  async function save() {
    ...
    if (!parsed.success || parsed.data.start_attendance === parsed.data.end_attendance) {
      setFeedback({ type: "error", message: t("Use valid times, duration, counts, and attendance day range (1–31).") });
      return;
    }
    ...
    try {
      ...
      const payload: unknown = await response.json().catch(() => null);
      if (!response.ok) throw new Error(typeof payload === "object" && payload && "error" in payload && typeof payload.error === "string" ? payload.error : t("Settings could not be saved."));
      const updated = settingsSchema.parse(payload);
      setValues(updated); setFeedback({ type: "success", message: t("Settings saved.") });
    } catch (error) {
      setFeedback({ type: "error", message: translate(error instanceof Error ? error.message : t("Settings could not be saved.")) });
  ...
  return (
    ...
    <section className="border border-[var(--color-border)] bg-[rgba(242,182,61,.025)]">
      ...
      <div className="flex flex-col gap-3 border-t border-[var(--color-border)] p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
        ...
        <Button loading={saving} disabled={saving || !values.is_admin} onClick={save} className="h-11 px-6 font-extrabold uppercase">{values.is_admin ? t("Save settings") : t("Admin required")}</Button>
      </div>
    </section></div>
  );
}
