...
const statusStyles: Record<AttendanceDayStatus, { card: string; dot: string; count: string; label: string }> = {
  good: { card: "border-[rgba(242,182,61,.3)] bg-[rgba(242,182,61,.06)]", dot: "bg-[var(--color-primary)]", count: "text-[var(--color-primary)]", label: "text-[var(--color-primary)]" },
  safe: { card: "border-[rgba(85,223,189,.28)] bg-[rgba(42,211,169,.06)]", dot: "bg-[#55dfbd]", count: "text-[#55dfbd]", label: "text-[#55dfbd]" },
  danger: { card: "border-[rgba(239,116,116,.3)] bg-[rgba(239,116,116,.06)]", dot: "bg-[#ef7474]", count: "text-[#ef7474]", label: "text-[#ef7474]" },
  upcoming: { card: "border-[var(--color-border)] bg-[rgba(255,255,255,.012)]", dot: "bg-[rgba(185,172,145,.35)]", count: "text-[var(--color-foreground-muted)]", label: "text-[var(--color-foreground-muted)]" },
};
...
export function AttendanceCalendarView({ initialData, playerThreshold, today }: Props) {
  ...
  async function changeMonth(offset: number) {
    ...
    setError(false);
    try {
      const response = await fetch(`/api/attendance?month=${encodeURIComponent(target)}`, { cache: "no-store", signal: controller.signal });
  ...
  return (
    <DashboardPage description="Daily turnout across the contract period, measured against the player threshold." eyebrow="Member records" title="Attendance Calendar">
      <section className="mt-[30px] grid gap-3 sm:grid-cols-3">
        <LegendCard label={t("Safe")} note={t("Above {count} players", { count: playerThreshold })} status="safe" value={summary.safe} />
        <LegendCard label={t("Good")} note={t("Exactly {count} players", { count: playerThreshold })} status="good" value={summary.good} />
        <LegendCard label={t("Danger")} note={t("Below {count} players", { count: playerThreshold })} status="danger" value={summary.danger} />
      </section>
      ...
      <section className="mt-4 border border-[var(--color-border)] bg-[rgba(255,255,255,.012)]">
        <div className="flex flex-col items-start gap-2 border-b border-[var(--color-border)] px-4 py-3 text-sm sm:flex-row sm:items-center sm:gap-3">
          <span className="flex items-center gap-3"><i className="h-2 w-2 shrink-0 rounded-full bg-[#55dfbd] shadow-[0_0_10px_rgba(85,223,189,.5)]" /><strong className="whitespace-nowrap uppercase">{t("Attendance summary by date")}</strong></span>
          <span className="whitespace-nowrap rounded-full bg-[rgba(255,255,255,.04)] px-2 py-1 text-xs text-[var(--color-foreground-muted)]">{t("{count} attendance days", { count: report.attendance_days.length })}</span>
          <div className="ml-auto flex items-center gap-2">
            <button aria-label={t("Previous month")} className="grid h-9 w-9 place-items-center border border-[var(--color-border)] text-[var(--color-primary)] disabled:opacity-40" disabled={loading} onClick={() => void changeMonth(-1)} type="button">‹</button>
            ...
            <button aria-label={t("Next month")} className="grid h-9 w-9 place-items-center border border-[var(--color-border)] text-[var(--color-primary)] disabled:opacity-40" disabled={loading} onClick={() => void changeMonth(1)} type="button">›</button>
          </div>
        </div>
        {error ? <Alert className="m-3" type="error" showIcon title={t("Could not load selected month.")} /> : null}
        <div className={`overflow-x-auto p-4 transition-opacity sm:p-5 ${loading ? "opacity-45" : "opacity-100"}`} aria-busy={loading}>
          <div className="grid min-w-[760px] grid-cols-7 gap-3">
            {weekdayHeadings(locale).map((weekday) => (
              <span className="pb-1 text-center text-xs font-black tracking-[.14em] text-[var(--color-primary-muted)] uppercase" key={weekday}>{weekday}</span>
            ))}
            {weeks.flatMap((week, weekIndex) => week.map((day, dayIndex) => {
              ...
              return (
                <button aria-label={t("Show attendance detail")} className={`cursor-pointer border p-3.5 text-left transition-colors hover:border-[var(--color-primary-muted)] ${style.card}`} key={day.date} onClick={() => setSelectedDate(day.date)} type="button">
                  <div className="flex items-start justify-between gap-2">
                    <span className="text-xs tracking-[.14em] text-[var(--color-foreground-muted)] uppercase">{formatMonth(day.date, locale)}</span>
                    <i className={`mt-1 h-2 w-2 shrink-0 rounded-full ${style.dot}`} aria-hidden="true" />
                  </div>
                  <strong className="mt-1 block font-[Impact] text-3xl leading-none font-normal">{Number(day.date.slice(-2))}</strong>
                  <p className="mt-2.5 text-sm">
                    <span className={`font-bold ${style.count}`}>{day.present}</span>
                    <span className="text-[var(--color-foreground-muted)]"> / {day.roster}</span>
                  </p>
                  <p className={`mt-1 text-xs font-extrabold tracking-[.12em] uppercase ${style.label}`}>{translate(statusLabels[day.status])}</p>
                </button>
              );
            }))}
          </div>
...
function LegendCard({ label, note, status, value }: { label: string; note: string; status: AttendanceDayStatus; value: number }) {
...
function formatMonth(date: string, locale: "en" | "id") {
  return new Intl.DateTimeFormat(locale === "id" ? "id-ID" : "en", { month: "short", timeZone: "UTC" }).format(new Date(`${date}T00:00:00Z`));
}
