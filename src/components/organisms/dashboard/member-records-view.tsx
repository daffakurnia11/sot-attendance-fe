...
export function MemberRecordsView({ data }: { data: MemberRecords | null }) {
  ...
  if (!data) return <Alert type="error" showIcon title="Personal records could not be loaded." />;
  ...
  return <>
    ...
    <div className="mt-6"><DataTable title="Player Logs" code="PL" columns={[{ label: "Status" }, { label: "Started" }, { label: "Occurred" }, { label: "Playtime" }]} empty="No player activity recorded." footer={<TablePagination onPageChange={setPlayerPage} page={playerPage} total={data.player_logs.length} />}>
      {paginateItems(data.player_logs, playerPage).map((log) => <tr className={dataTableRowClassName} key={log.id}><DataTableCell><Status value={log.status} /></DataTableCell><DataTableCell>{formatDateTime(log.started_at)}</DataTableCell><DataTableCell>{formatDateTime(log.occurred_at)}</DataTableCell><DataTableCell>{log.playtime_seconds === null ? "—" : formatDuration(log.playtime_seconds)}</DataTableCell></tr>)}
    </DataTable></div>
...
function formatDate(value: string) { return new Intl.DateTimeFormat("en-GB", { dateStyle: "medium", timeZone: "Asia/Jakarta" }).format(new Date(value)); }
function formatTime(value: string) { return new Intl.DateTimeFormat("en-GB", { hour: "2-digit", minute: "2-digit", hour12: false, timeZone: "Asia/Jakarta" }).format(new Date(value)); }
function formatDateTime(value: string | null) { return value ? `${formatDate(value)}, ${formatTime(value)}` : "—"; }
