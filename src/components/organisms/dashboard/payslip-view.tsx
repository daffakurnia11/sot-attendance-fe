...
export function PayslipView({ initialData }: { initialData: PayslipReport | null }) {
  ...
  async function changeMonth(offset: number) {
    ...
    setError(false);
    try {
      const response = await fetch(`/api/payslips?month=${encodeURIComponent(target)}`, {
        cache: "no-store",
  ...
  if (!report)
    return (
      <div className="w-full px-3.5 pt-6 sm:px-6 sm:pt-[30px]">
        <Alert type="error" showIcon title={t("Payslip data could not be loaded.")} />
      </div>
    );
  ...
  const toolbar = (
    <div className="flex flex-wrap items-center gap-3">
      <label className="sr-only" htmlFor="payslip-search">
        {t("Search players")}
      </label>
      <input
        aria-label={t("Search players...")}
        className="h-10 min-w-[220px] flex-1 border border-[var(--color-border)] bg-[rgba(255,255,255,.015)] px-3 text-base outline-none placeholder:text-[var(--color-foreground-muted)] focus:border-[var(--color-primary-muted)]"
        id="payslip-search"
  ...
  return (
    ...
    >
      <section className="mt-[30px]">
        <SectionHeader eyebrow={t("Overview")} index="01" title={t("Payslip Statistics")} />
        ...
        >
          {players.map((player, index) => (
            <tr className={dataTableRowClassName} key={player.member_id}>
              ...
              <DataTableCell>
                <span className={player.eligible ? "text-[#55dfbd]" : "text-[#ef7474]"}>
