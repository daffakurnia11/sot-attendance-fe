...
describe("fetchPayslips", () => {
  ...
  it("rejects unsafe amount shapes", async () => {
    ...
    await expect(fetchPayslips("http://api.test", "token", undefined, fetcher as typeof fetch)).rejects.toThrow(
      "invalid data",
    );
  });
});
...
const player = (member_id: number, attended_days: number, payout: string, eligible = true) => ({
  member_id,
  username: `u${member_id}`,
  display_name: `D${member_id}`,
  character_name: `C${member_id}`,
  attended_days,
  eligible,
  payout,
});
