...
export const memberProfileSchema = z.object({ character_name: z.string().trim().min(1).max(80).refine((value) => !/[\r\n\0]/.test(value)) });
export type MemberProfile = z.infer<typeof memberProfileSchema>;

export async function updateMemberProfile(baseURL: string, accessToken: string, profile: MemberProfile, fetcher: typeof fetch = fetch) {
  const response = await fetcher(new URL("/api/v1/me/profile", baseURL), {
    ...
    body: JSON.stringify(profile), cache: "no-store", signal: AbortSignal.timeout(5_000),
  });
